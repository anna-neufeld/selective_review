...
for (t in 1:ntrials) {
  ...
  if (min(table(est.clust.null)) > 1) {
    xbar1 <- mean(X[est.clust.null==1,1])
    xbar2 <- mean(X[est.clust.null==2,1])
    n1 <- sum(est.clust.null==1)
    n2 <- sum(est.clust.null==2)
    CI.naive <- c(
      (xbar1 - xbar2) - qnorm(0.95)*sigma*sqrt(1/n1+1/n2),
      (xbar1 - xbar2) + qnorm(0.95)*sigma*sqrt(1/n1+1/n2)
    )
  ...
  if (min(table(est.clust.alt)) > 1) {
    ...
    xbar1 <- mean(X[est.clust.alt==1,1])
    xbar2 <- mean(X[est.clust.alt==2,1])
    n1 <- sum(est.clust.alt==1)
    n2 <- sum(est.clust.alt==2)
    CI.naive <- c(
      (xbar1 - xbar2) - qnorm(0.95)*sigma*sqrt(1/n1+1/n2),
      (xbar1 - xbar2) + qnorm(0.95)*sigma*sqrt(1/n1+1/n2)
    )
